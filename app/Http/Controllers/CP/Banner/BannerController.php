...
use App\Model\Banner\Main as Model;
use Image;
    ...
    public function update(Request $request){
        ...
        if($id == 1){
            if($request->hasFile('image')) {
                $image = $request->file('image');
                $imagename = time().'.'.$image->getClientOriginalExtension(); 
                Image::make($image->getRealPath())->resize(1900, 200)->save(public_path('uploads/banner/image/'.$imagename));
                $data['img_url']='public/uploads/banner/image/'.$imagename;
            }
        }elseif($id == 2){
            if($request->hasFile('image')) {
                $image = $request->file('image');
                $imagename = time().'.'.$image->getClientOriginalExtension(); 
                Image::make($image->getRealPath())->resize(1000, 200)->save(public_path('uploads/banner/image/'.$imagename));
                $data['img_url']='public/uploads/banner/image/'.$imagename;
            }
        }else{
            if($request->hasFile('image')) {
                $image = $request->file('image');
                $imagename = time().'.'.$image->getClientOriginalExtension(); 
                Image::make($image->getRealPath())->save(public_path('uploads/banner/image/'.$imagename));
                $data['img_url']='public/uploads/banner/image/'.$imagename;
            }        
	}
