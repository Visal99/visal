...
@section ('appbottomjs')
	...
	<script>
		...
		$("#kh_image").fileinput({
		    ...
		    elErrorContainer: '#kv-avatar-errors-2',
		    msgErrorClass: 'alert alert-block alert-danger',
		    defaultPreviewContent: '<img src="{{ asset ($data->kh_image) }}" alt="Missing Image" class="img img-responsive" ><span class="text-muted">Click to select <br /><i style="font-size:12px">Image dimesion must be {{ $data->width }}cmX{{ $data->height }}cm with .jpg or .png type</i></span>',
		    layoutTemplates: {main2: '{preview} ' +  btnCust + ' {remove} {browse}'},
		    allowedFileExtensions: ["jpg", "png", "gif"]
		});
	</script>
						...
						@if(isset($_GET['page']))
							@if($_GET['page'] != "")
							    ...
							    @if(isset($_GET['menu']))
							        @php( $menu = $_GET['menu'])
							    @endif
								<a href="{{ route('user.content.list', ['page' => $_GET['page']]) }}?menu={{$menu}}&page={{ $_GET['page'] }}" class="btn"><i class="fa fa-arrow-left"></i></a>
							@endif
						@endif
					</div>
				</div>
	...
	<div class="container-fluid">
			<div class="box-typical box-typical-padding">
				<div class="container-fluid">
					...
					<form id="form" action="{{ route('user.content.update') }}?" name="form" method="POST"  enctype="multipart/form-data">
						{{ csrf_field() }}
						{{ method_field('POST') }}
