				...
				<div class="container-fluid">
	                ...
	                @php ($active = 0)
	               	@if (Session::has('invalidData'))
		                ...
		                @php ($position_id = $invalidData['position_id'])
		                @php ($active = $invalidData['active'])
	               	@endif
					<form id="form" action="{{ route('user.user.store') }}" name="form" method="POST"  enctype="multipart/form-data">
						{{ csrf_field() }}
						{{ method_field('PUT') }}
						...
						<div class="form-group row">
							<label class="col-sm-2 form-control-label" for="name">Password</label>
							<div class="col-sm-10">
								<input 	id="password"
										name="password"
									   	value = ""
									   	...
									   	class="form-control"
									   	data-validation="[L>=6]"
										data-validation-message="$ must be at lest 6 characters." />
										
							</div>
						</div>
						<div class="form-group row">
							<label class="col-sm-2 form-control-label" for="name">Confirmed Password</label>
							<div class="col-sm-10">
						...
						<div class="form-group row">
							...
							<div class="col-sm-10">
								<div class="checkbox-toggle">
									<input name="status" id="status" type="checkbox"  @if($active ==1 ) checked @endif >
