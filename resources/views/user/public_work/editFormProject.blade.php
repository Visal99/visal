...
@section ('appbottomjs')
	...
	<script>
		...
		$("#image").fileinput({
		    ...
		    defaultPreviewContent: '<img src="{{ asset($project->image) }}" alt="Missing Image" class="img img-responsive"><span class="text-muted">Click to select <br /><i style="font-size:12px">Image dimesion must be (100-500)cmX(100-500)cm with .jpg or .png type</i></span>',
			...
			<div class="tbl">
				<div class="tbl-row">
					...
					<div class="tbl-cell tbl-cell-action">
						<a href="{{ route('user.public-work.list-project',$data->id) }}"  class="tabledit-delete-button btn btn-sm btn-primary" style="float: none;"><span class="fa fa-arrow-left"></span></a>
						<a href="{{ route('user.public-work.create-project',$data->id) }}"  class="tabledit-delete-button btn btn-sm btn-primary" style="float: none;"><span class="fa fa-plus"> Add New Project</span></a>
					</div>
				</div>
			</div>
		</div>
				...
				<div class="tabs-section-nav tabs-section-nav-icons">
					<div class="tbl">
						<ul class="nav" role="tablist">
							...
							<li class="nav-item">
								<a class="nav-link active" onclick="window.location.href='{{ route('user.public-work.list-project', $data->id) }}'" href="#" role="tab" data-toggle="tab">
									<span class="nav-link-in">
							...
							<li class="nav-item">
								<a class="nav-link" onclick="window.location.href='{{ route('user.public-work.list-document', ['id'=>$data->id,'slug'=>$data->slug]) }}'" href="#" role="tab" data-toggle="tab">
									<span class="nav-link-in">
										<span class="glyphicon glyphicon-home"></span>
										Regulation
									</span>
					...
					@endif
					<form id="form" action="{{ route('user.public-work.update-project',$data->id) }}" name="form" method="POST"  enctype="multipart/form-data">
						{{ csrf_field() }}
						{{ method_field('POST') }}
							...
							<div class="col-sm-10">
								<input 	id="en_construction_type"
										name="en_construction_type"
									   	value = "{{ $project->en_construction_type }}"
									   	type="text"
									   	placeholder = "Eg. Jhon Son"
									   	class="form-control"
									   	data-validation="[L>=4, L<=50]"
							...
							<div class="col-sm-10">
								<input 	id="kh_construction_type"
										name="kh_construction_type"
									   	value = "{{ $project->kh_construction_type }}"
									   	type="text"
									   	placeholder = "Eg. Title "
									   	class="form-control"
									   	data-validation="[L>=4, L<=50]"
						...
						<div class="form-group row">
							...
							<div class="col-sm-10">
								<div class="summernote-theme-1">
									<textarea id="en_background" name="en_background" class="form-control  ">{{$project->en_background}} </textarea>
								</div>	
							</div>
						</div>
						<div class="form-group row">
							...
							<div class="col-sm-10">
								<div class="summernote-theme-1">
									<textarea id="kh_background" name="kh_background" class="form-control  ">{{$project->kh_background}} </textarea>
								</div>	
							</div>
						</div>
						...
						<div class="form-group row">
							...
							<div class="col-sm-10">
								<div class="checkbox-toggle">
									<input name="feature" id="feature" type="checkbox"  @if($project->featured ==1 ) checked @endif >
						...
						<div class="form-group row">
							<label class="col-sm-2 form-control-label" for="en_note">Note (Eng)</label>
							<div class="col-sm-10">
								<div class="summernote-theme-1">
									<textarea id="en_note" name="en_note" class="form-control  ">{{$project->en_note}} </textarea>
								</div>	
							</div>
						</div>
						<div class="form-group row">
							<label class="col-sm-2 form-control-label" for="kh_note">Note (KHM)</label>
							<div class="col-sm-10">
								<div class="summernote-theme-1">
									<textarea id="kh_note" name="kh_note" class="form-control  ">{{$project->kh_note}} </textarea>
								</div>	
							</div>
						</div>
						<div class="form-group row">
							...
							<div class="col-sm-10">
								...
								<button type="button" onclick="deleteConfirm('{{ route('user.public-work.trash-project', $data->id) }}', '{{ route('user.public-work.list-project',$data->id) }}')" class="btn btn-danger"> <fa class="fa fa-trash"></i> Delete</button>
							</div>
						</div>
					</form>
				</div>
