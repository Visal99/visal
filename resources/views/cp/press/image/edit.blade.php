...
@section ('tab-active-image', 'active')
...
@section ('section-js')
	<script>
		...
		$("#image").fileinput({
		    ...
		    defaultPreviewContent: '<img src="{{ asset($data->img_url) }}" alt="Missing Image" class="img img-responsive"><span class="text-muted">Click to select <br /><i style="font-size:12px">Image dimesion must be 960x640 with .jpg or .png type</i></span>',
	...
	<form id="form" action="{{ route($route.'.update') }}" name="form" method="POST"  enctype="multipart/form-data">
			...
			<div class="col-sm-10">
				<input 	id="en_title"
						name="en_title"
					   	value = "{{$data->en_title}}"
					   	type="text"
					   	placeholder = "Eg. Enter Title in English."
					   	class="form-control"
					   	 />
			...
			<div class="col-sm-10">
				<input 	id="kh_title"
						name="kh_title"
					   	value = "{{$data->kh_title}}"
					   	type="text"
					   	placeholder = "Eg. Enter Title in Khmer. "
					   	class="form-control"
					   	 />
		...
		<div class="form-group row">
			...
			<div class="col-sm-10">
				...
				<button type="button" onclick="deleteConfirm('{{ route($route.'.trash', ['id'=>$id, 'image_id'=>$data->id]) }}', '{{ route($route.'.index', $id) }}')" class="btn btn-danger"> <fa class="fa fa-trash"></i> Delete</button>
			</div>
		</div>
	</form>
