...
@section ('tab-js')
	<script type="text/javascript">
			...
			function updateFeatured(id){
		     	...
		     	$.ajax({
			        url: "{{ route($route.'.image-featured') }}",
			        method: 'POST',
			        data: {id:id, active:active, news_id:news_id },
			        success: function( response ) {
			            if ( response.status === 'success' ) {
			            	swal("Nice!", response.msg ,"success");
			            	location.reload();
			...
			<tbody>
				...
				@foreach ($data as $row)
					<tr id="element-{{ $row->id }}" data-id="{{ $row->id }}" class="moveable" feature-order="{{ $row->data_order }}" draggable="true" ondragenter="dragenter(event)" ondragstart="dragstart(event)" ondragend="dragend(event)">
						...
						<td>{{ $row->en_title }}</td>
						<td>
								<img style=" width: 200px;" src="{{ asset ($row->img_url) }}" alt="" data-toggle="tooltip" data-placement="bottom" title="{{ $row->en_title }}">
						</td>
						...
						<td style="white-space: nowrap; width: 1%;">
							<div class="tabledit-toolbar btn-toolbar" style="text-align: left;">
	                           	<div class="btn-group btn-group-sm" style="float: none;">
	                           		...
	                           		<a href="#" onclick="deleteConfirm('{{ route($route.'.trash', ['id'=>$id, 'image_id'=>$row->id]) }}', '{{ route($route.'.index', ['id'=>$id]) }}')" class="tabledit-delete-button btn btn-sm btn-danger" style="float: none;"><span class="glyphicon glyphicon-trash"></span></a>
