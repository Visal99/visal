@extends($route.'.main')
@section ('section-title', 'All Public Services')
...
@section ('section-js')
	...
	<script type="text/javascript">
			...
			function dragend(e){
				...
				console.log(string);
				$.ajax({
				        url: "{{ route($route.'.order-automation') }}",
				        type: 'POST',
				        data: {string:string},
				        success: function( response ) {
				         	if ( response.status === 'success' ) {
		...
		<tbody>
			...
			@foreach ($data as $row)
				<tr id="element-{{ $row->id }}" data-id="{{ $row->id }}" class="moveable" feature-order="{{ $row->data_order }}" draggable="true" ondragenter="dragenter(event)" ondragstart="dragstart(event)" ondragend="dragend(event)" >
					...
					<td>
							<img src="{{ asset ($row->icon) }}" alt="" data-toggle="tooltip" data-placement="bottom" title="{{ $row->en_title }}">
					</td>
