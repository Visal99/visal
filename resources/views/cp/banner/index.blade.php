@extends($route.'.main')
@section ('section-title', 'All Website')
		...
		<tbody>
			...
			@foreach ($data as $row)
				<tr id="element-{{ $row->id }}" data-id="{{ $row->id }}" class="moveable" feature-order="{{ $row->featured_order }}" draggable="true" ondragenter="dragenter(event)" ondragstart="dragstart(event)" ondragend="dragend(event)">
					...
					<td>{{ $row->en_title }}</td>
					<td>
							<img src="{{ asset ('public/uploads/website/image/'.$row->img_url) }}" alt="" data-toggle="tooltip" data-placement="bottom" title="{{ $row->en_title }}">
					</td>
